	...
	for id, s in pairs(scores) do
		if id ~= -1 then
			...
			if foundPlayer then
				...
				if role == "inn" or role == "det" or role == "mer" or role == "pha" or role == "gli" or role == "emt" then
...
function CLSCORE:Init(events)
	...
	scores = ScoreEventLog(events, scores, traitors, detectives, hypnotist, mercenary, jester, phantom, glitch, zombie, vampire, swapper, assassin, killer, emt)
